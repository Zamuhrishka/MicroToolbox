...
extern "C"
{
    ...
    static uint8_t shadow[McuASAN_CONFIG_APP_MEM_SIZE / 8]; /* one shadow byte for 8 application memory bytes. A 1 means
                                                               that the memory address is poisoned */
    ...
    {
        if (addr >= (void *)McuASAN_CONFIG_APP_MEM_START &&
            addr < (void *)(McuASAN_CONFIG_APP_MEM_START + McuASAN_CONFIG_APP_MEM_SIZE))
        {
    ...
    {
        printf("ASAN ptr failure: addr 0x%x, %s, size: %d", address, mode == kIsRead ? "read" : "write", kAccessSize);
    ...
    static void CheckShadow(void *address, size_t kAccessSize, rw_mode_e mode)
    {
        ...
        if (address >= (void *)McuASAN_CONFIG_APP_MEM_START &&
            address < (void *)(McuASAN_CONFIG_APP_MEM_START + McuASAN_CONFIG_APP_MEM_SIZE))
        {
    ...
    {
        CheckShadow(address, 8, kIsRead); /* check if we are reading from poisoned memory */
    }

    void __asan_store8_noabort(void *address)
    {
        CheckShadow(address, 8, kIsWrite); /* check if we are writing to poisoned memory */
    }
